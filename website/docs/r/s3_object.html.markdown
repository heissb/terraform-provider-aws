...
* `checksum_sha256` - (Optional) The base64-encoded, 256-bit SHA-256 digest of the object. If the specified digest differs from that of the object body, the AWS API rejects the request and the apply operation fails. Set using `filebase64sha256("path/to/source")`. Useful for triggering an update when the source file changes. If this argument is specified, `checksum_algorithm` must be set to `SHA256`.
